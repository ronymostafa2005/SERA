...
export default function IbtehalatNashqbandi() {
    ...
    // ========= to fech data from the API ================
    useEffect(() => {
        fetch("/api/ibtehalat").then((res) => res.json())
    ...
    return (
        <>
            <Sidebar />
               ...
               {!loading && ibtehalatData.length === 0 && <p className="text-gray-400 text-lg">لا توجد إبتهالات متاحة</p>}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {currentItems.map((item) => (
                        <div key={item.title} className="bg-slate-800 p-4 rounded-xl border border-emerald-500/20 hover:border-emerald-400 cursor-pointer transition-all transform hover:scale-[1.02] hover:shadow-[0_0_15px_rgba(16,185,129,0.5)]">
